...
#Plots of biomass will be saved to a new folder "temp_imgs"
...
#################################################################
#Set the output path
wdir = "C:\\Users\\sflanagan\\desktop\\landis_runs\\100m\\Output\\"

#List the scenarios you want to examine
scen_list = ["base_case","rBiomass_iMor_newAges"]  
...
num_regions = 9
species =["nyssbifl","pinuelli","pinupalu","querfalc","querhemi",
...
for i in range(0,len(scen_list)):
    ...
    if len(scen_list)==1 and i==0:
        ...
        odir = wdir+"temp_imgs" + "\\" + scen_list[i]
        oedir = odir+"\\ecoregions"
        os.makedirs(odir)
        os.makedirs(oedir)
    ...
    for j in range(0,len(species)):
        ...
        nf = ff.groupby(["Time"])[s_name].sum().reset_index()
        nf.plot("Time",s_name)
        plt.ylabel("AGB (g/m2)")
        plt.xlabel("Time (yr)")
    ...
    for j in range(0,len(species)):
        ...
        #Get time and total biomass summed over ecoregions        
        nf = ff.groupby(["Time"])[s_name].sum().reset_index()
